...
def _json_safe_status_value(value, default=None):
    """Return a JSON-serializable status value with mock-safe fallback."""
    if value is None:
        return default
...
def _scanner_status_payload(scanner) -> dict[str, object]:
    """Build a stable status payload from scanner attributes/status."""
    status_obj = None
    with contextlib.suppress(Exception):
        status_obj = scanner.get_status()

    payload = {
        "is_scanning": bool(getattr(scanner, "is_scanning", False)),
        "mode": _json_safe_status_value(getattr(scanner, "scan_mode", None)),
        "backend": None,
        "adapter_id": None,
        "device_count": _json_safe_status_value(getattr(scanner, "device_count", 0), 0),
    }

    if status_obj is not None:
        payload["mode"] = _json_safe_status_value(getattr(status_obj, "mode", None), payload["mode"])
        payload["backend"] = _json_safe_status_value(getattr(status_obj, "backend", None))
        payload["adapter_id"] = _json_safe_status_value(getattr(status_obj, "adapter_id", None))
        payload["device_count"] = _json_safe_status_value(
            getattr(status_obj, "device_count", None),
            payload["device_count"],
        )
        payload["error"] = _json_safe_status_value(getattr(status_obj, "error", None))

    if hasattr(scanner, "get_baseline_count"):
        with contextlib.suppress(Exception):
            payload["baseline_count"] = int(scanner.get_baseline_count())

    return payload


def device_to_dict(device: BTDeviceAggregate) -> dict:
    """Backward-compatible device serializer."""
    payload = device.to_dict()
    heuristics = payload.get("heuristics", {})
    payload.setdefault("is_new", heuristics.get("is_new"))
    payload.setdefault("is_persistent", heuristics.get("is_persistent"))
    payload.setdefault("is_beacon_like", heuristics.get("is_beacon_like"))
    payload.setdefault("is_strong_stable", heuristics.get("is_strong_stable"))
    payload.setdefault("has_random_address", heuristics.get("has_random_address"))
    return payload
...
def start_scan():
        ...
        return jsonify(
            {
                ...
                "scan_status": _scanner_status_payload(scanner),
            }
        )
    ...
    try:
        ...
        logger.error(f"Bluetooth v2 start_scan failed: {e}")
        return jsonify(
            {
                "status": "error",
                "message": str(e) or "Failed to start scan",
            }
        )

    status_payload = _scanner_status_payload(scanner)
    if success:
        return jsonify(
            {
                "status": "started",
                ...
                "adapter_id": status_payload.get("adapter_id"),
            }
        )
    ...
    return jsonify(
        {
            "status": "error",
            "message": status_payload.get("error") or "Failed to start scan",
        }
    )
...
def get_scan_status():
    ...
    """
    scanner = get_bluetooth_scanner()
    return jsonify(_scanner_status_payload(scanner))
