    ...
    def start_from_file(
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(
                    f"Weather satellite file decode started: {satellite} " f"({sat_info['mode']}) from {input_file}"
                )
                self._emit_progress(
                    CaptureProgress(
                        status="decoding",
            ...
            for ext in ("*.png", "*.jpg", "*.jpeg"):
                for filepath in self._capture_output_dir.rglob(ext):
                    ...
                    if suffix not in (".png", ".jpg", ".jpeg"):
                        suffix = ".png"
                    serve_name = f"{safe_sat}_{safe_stem}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}" f"{suffix}"
