...
def require_tool(*names: str) -> str:
    """Return the first available tool path or skip the test."""
    for name in names:
        path = shutil.which(name)
        if path:
            return path
    pytest.skip(f"Required tool not installed: {' / '.join(names)}")
...
class TestToolDetection:
    ...
    def test_rtl_433_available(self):
        """rtl_433 should be detectable when installed."""
        assert require_tool("rtl_433")

    def test_dump1090_available(self):
        """dump1090/readsb should be detectable when installed."""
        assert require_tool("dump1090", "dump1090-fa", "readsb")

    def test_airodump_available(self):
        """airodump-ng should be detectable when installed."""
        assert require_tool("airodump-ng")

    def test_multimon_available(self):
        """multimon-ng should be detectable when installed."""
        assert require_tool("multimon-ng")

    def test_acarsdec_available(self):
        """acarsdec should be detectable when installed."""
        assert require_tool("acarsdec")

    def test_agent_detects_tools(self, agent):
        """Agent should detect available tools."""
        caps = agent.detect_capabilities()

        assert "sensor" in caps["modes"]
        assert "adsb" in caps["modes"]
        assert isinstance(caps["modes"]["sensor"], bool)
        assert isinstance(caps["modes"]["adsb"], bool)
...
class TestRTLSDRDetection:
    ...
    def test_rtl_test_runs(self):
        ...
        rtl_test_path = require_tool("rtl_test")
        result = subprocess.run([rtl_test_path, "-t"], capture_output=True, timeout=5)
...
class TestLiveRTL433:
    ...
    def test_rtl_433_runs(self):
        ...
        rtl_433_path = require_tool("rtl_433")
        proc = subprocess.Popen(
            [rtl_433_path, "-F", "json", "-T", "3"], stdout=subprocess.PIPE, stderr=subprocess.PIPE  # Run for 3 seconds
        )
    ...
    def test_rtl_433_json_output(self):
        ...
        rtl_433_path = require_tool("rtl_433")
        proc = subprocess.Popen([rtl_433_path, "-F", "json", "-T", "5"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class TestLiveAgentModes:
    ...
    def test_agent_sensor_mode(self, agent):
        ...
        if result.get("status") == "error":
            ...
            pytest.skip(f"Sensor mode not runnable in this environment: {message}")
    ...
    def test_agent_adsb_mode(self, agent):
        ...
        if result.get("status") == "error":
            ...
            if "device" in message.lower():
                pytest.skip("No RTL-SDR device")
            pytest.skip(f"ADS-B mode not runnable in this environment: {message}")
